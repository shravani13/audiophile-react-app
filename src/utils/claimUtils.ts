...
export const createClaimsTable = (claims:IdTokenClaims) => {
    let claimsObj:ClaimType[] = [];
    if(!claims) return claimsObj;
    let index = 0;

    Object.keys(claims).forEach((key:string|number) => {
        ...
        switch (key) {
            ...
            case 'iat':
                populateClaim(
                    key,
                    changeDateFormat(value as number),
            ...
            case 'acct':
                ...
                break;
            case 'sid':
                populateClaim(key, value as string, 'Session ID, used for per-session user sign-out.', index, claimsObj);
                index++;
                break;
            case 'sub':
                populateClaim(
            ...
            case '_claim_sources':
            default:
                populateClaim(key, value as string, '', index, claimsObj);
                index++;
        }
    });
 ...
 */
const populateClaim = (claim:string|number, value:string, description:string, index:number, claimsObject:ClaimType[]) => {
    let claimObject:ClaimType = {
        claim: claim,
        value: value,
        description: description
    }
    claimsObject[index] = claimObject;
};
